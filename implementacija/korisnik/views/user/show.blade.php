      ...
      <div class="moves container">
        <div class="thumbnail" style="width: 20rem; height: 27rem;">
          <div class="caption">
            ...
            <div>
              <form method='POST' action='/profile/1/feed?pokemon={{ $data->first()->pokemon_id }}' style="float:left;">
                @csrf
                @method('PUT')
                <button type='submit' class="btn btn-primary @if($user->cntFruits==0) disabled @endif">Feed</a>
              </form>

              <form method='POST' action='/profile/1/release?pokemon={{ $data->first()->pokemon_id }}' style="float:left;">
                @csrf
                @method('PUT')
